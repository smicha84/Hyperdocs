...
def primitives_tagger_prompt(session_id, safe_condensed, safe_tier4, session_summary, subset_indices=None):
    ...
    if chunks:
        ...
        for ci, chunk in enumerate(chunks):
            ...
            prompt = _prepend_commitments(thread_analyst_prompt(
                session_id, chunk_data["safe_condensed"], chunk_data["safe_tier4"], chunk_data["session_summary"]
            ))
            r = call_opus(prompt)
            if r:
        ...
        if prim_result:
            ...
            while n_tagged < expected_count and n_tagged > 0 and continuation_round < max_continuations:
                ...
                cont_prompt = _prepend_commitments(primitives_tagger_prompt(
                    ...
                    data["session_summary"], subset_indices=remaining_indices
                ))
