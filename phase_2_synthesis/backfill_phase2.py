...
logger = get_logger("phase2.backfill_phase2")
...
def find_session_dir(sid):
    """Find the session directory matching the given session ID prefix."""
    for entry in os.listdir(BASE):
        if entry.startswith(f"session_{sid}") and os.path.isdir(os.path.join(BASE, entry)):
            return os.path.join(BASE, entry)
    return None

def _extract_canonical_thread_nodes(thread_data, nodes, edges):
    """Extract idea nodes from canonical thread format (dict with category sub-keys)."""
...
def _extract_old_format_nodes(threads, nodes, edges):
    """Extract idea nodes from old extractions-list thread format."""
    ...
    for ext in threads.get("extractions", []):
        idx = ext.get("index", 0)
        t = ext.get("threads", {})
        idea = t.get("user_ideas", {}).get("idea")
        if idea and len(idea) > 20:
            nid = f"idea_msg_{idx}"
            frustration = ext.get("markers", {}).get("frustration_level", 0)
            nodes.append({
                "id": nid,
                "name": idea[:80],
                "description": idea,
                "first_appearance": idx,
                "confidence": "working",
                "emotional_context": "frustrated" if frustration > 2 else "confident",
...
def build_idea_graph(sid, sdir, summary, threads, geo, prims, explorer):
    """Build an idea evolution graph from thread extractions and geological notes."""
    nodes = []
    edges = []

    if threads:
        # Try canonical format first: threads as dict with category sub-keys
        ...
        if isinstance(thread_data, dict) and thread_data:
            ...
            if isinstance(first_val, dict) and ('entries' in first_val or 'description' in first_val):
                _extract_canonical_thread_nodes(thread_data, nodes, edges)

        # Fall back to old format: extractions list
        if not nodes and threads.get("extractions"):
            _extract_old_format_nodes(threads, nodes, edges)
...
def build_synthesis(sid, sdir, summary, threads, geo, prims, explorer, idea_graph):
    """Build a 6-pass synthesis from session data and idea graph."""
...
def build_grounded_markers(sid, sdir, summary, threads, geo, prims, explorer, idea_graph, synthesis):
    """Build grounded markers from file activity, frustration events, and idea nodes."""
    markers = []
    stats = (summary or {}).get("session_stats", {})
    top_files = stats.get("top_files", [])
    frustration_peaks = stats.get("frustration_peaks", [])
...
def process_session(sid):
    """Process a single session: build missing Phase 2 files (idea graph, synthesis, markers)."""
    sdir = find_session_dir(sid)
    if not sdir:
        logger.warning(f"  SKIP {sid}: directory not found")
    ...
    if has_ig and has_syn and has_gm:
        logger.warning(f"  SKIP {sid}: all 3 P2 files exist")
    ...
    if has_ig:
        ...
        logger.info(f"  WROTE {sid}/idea_graph.json ({len(idea_graph.get('nodes', []))} nodes)")
    ...
    if has_syn:
        ...
        logger.info(f"  WROTE {sid}/synthesis.json")
    ...
    if not has_gm:
        ...
        logger.info(f"  WROTE {sid}/grounded_markers.json ({gm['total_markers']} markers)")

    return result

def main():
    """Run batch Phase 2 generation for all sessions in the SESSIONS list."""
    ...
    logger.info(f"\n=== Summary ===")
    logger.info(f"Files written: {total_wrote}")
    logger.warning(f"Sessions skipped: {total_skipped}")
