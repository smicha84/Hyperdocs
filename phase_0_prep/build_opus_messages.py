...
Usage:
    python3 build_opus_messages.py --session 0012ebed
    python3 build_opus_messages.py --dir ~/PERMANENT_HYPERDOCS/sessions/session_0012ebed
"""
from tools.log_config import get_logger

logger = get_logger("phase0.build_opus_messages")
...
def build_opus_filtered(session_dir: Path):
    ...
    if not cls_path.exists():
        logger.error(f"ERROR: opus_classifications.json not found in {session_dir}")
        logger.info("Run opus_classifier.py first.")
        return None
    ...
    if not enriched_path.exists():
        logger.error(f"ERROR: enriched_session.json not found")
        return None
    ...
    logger.info(f"Opus classifications: {len(opus_by_idx)}")
    ...
    logger.info(f"\nOpus distribution:")
    logger.info(f"  Critical:     {len(critical_msgs)}")
    logger.info(f"  Significant:  {len(significant_msgs)}")
    logger.info(f"  Context:      {len(context_msgs)}")
    logger.info(f"  Noise:        {len(noise_msgs)}")
    logger.info(f"  Unclassified: {len(unclassified_msgs)} (fell back to Python tier)")
    ...
    logger.info(f"  Python tier 4:     {len(python_tier4)} messages")
    logger.info(f"  Opus priority:     {len(priority)} messages (critical + significant)")
    logger.info(f"  NEW (Opus adds):   {len(new_in_priority)} messages")
    logger.info(f"  LOST (Opus drops): {len(lost_from_priority)} messages")

    # Write opus_priority_messages.json (replaces tier4_priority_messages.json)
    priority_output = {
        "session_id": cls_data.get("session_id", ""),
        "generated_at": datetime.utcnow().isoformat() + "Z",
        "generator": "build_opus_messages.py",
    ...
    logger.info(f"\nWritten:")
    logger.info(f"  {out_priority} ({out_priority.stat().st_size:,} bytes)")
    logger.info(f"  {out_extended} ({out_extended.stat().st_size:,} bytes)")
    logger.info(f"  {out_safe} ({out_safe.stat().st_size:,} bytes)")
...
def main():
    ...
    if result:
        logger.info(f"\n=== RESULT ===")
        logger.info(f"Opus priority: {result['priority_count']} messages ({result['critical']} critical + {result['significant']} significant)")
        logger.info(f"Opus extended: {result['extended_count']} messages (+ {result['context']} context)")
        logger.info(f"Delta from Python: +{result['new_messages']} new, -{result['lost_messages']} dropped")
