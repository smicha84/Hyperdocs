...
Usage:
    python3 batch_p0_llm.py                       # run all passes
    python3 batch_p0_llm.py --pass 1              # run only Pass 1
    python3 batch_p0_llm.py --dry-run             # estimate costs only
    python3 batch_p0_llm.py --session session_X   # one session only
    python3 batch_p0_llm.py --merge-only          # only merge step
    python3 batch_p0_llm.py --status              # show current progress
...
logger = get_logger("phase0.batch_p0_llm")
...
def run_batch_pass(sessions, pass_num, status, dry_run=False):
    ...
    if pass_num == 3:
        ...
        logger.info(f"    Model: {model}, Concurrency: {max_workers}")
        logger.info(f"    Targeted (with assumptions): {len(todo)}, Already done: {already_done}")
    else:
        logger.info(f"\n  Pass {pass_num}: {config['name']}")
        logger.info(f"    Model: {model}, Concurrency: {max_workers}")
        logger.info(f"    To process: {len(todo)}, Already done: {already_done}")

    if not todo:
        logger.info(f"    Nothing to do — all sessions already processed")
        return
...
def run_batch_merge(sessions, status):
    ...
    for i, session_dir in enumerate(todo):
        ...
        if (i + 1) % 20 == 0 or (i + 1) == len(todo):
            logger.info(f"    [{i+1}/{len(todo)}] Merged: {merged}, Errors: {merge_errors}")
    ...
    logger.info(f"    Merge done: {merged} ok, {merge_errors} errors")
...
def show_status():
    ...
    if not STATUS_FILE.exists():
        logger.info("No batch status file found. Run the orchestrator first.")
        return
    ...
    logger.info("=" * 60)
    logger.info("Phase 0.5 LLM Batch Status")
    logger.info("=" * 60)
    logger.info(f"  Started: {status.get('started_at', '?')}")
    logger.info(f"  Sessions: {status.get('total_sessions', '?')}")
    ...
    if status.get("completed_at"):
        ...
        logger.info(f"  Duration: {dur/60:.1f} minutes")
    logger.info("=" * 60)
...
def main():
    ...
    if args.session:
        ...
        logger.info(f"Single session mode: {session_dir.name}")
    else:
        sessions = get_unique_sessions()

    # ── Header ──
    logger.info("=" * 70)
    logger.info("Phase 0.5 LLM Batch Orchestrator")
    logger.info(f"  Sessions: {len(sessions)}")
    logger.info(f"  Started:  {datetime.now(timezone.utc).isoformat()}")
    if args.dry_run:
        logger.info("  MODE: DRY RUN (no API calls)")
    if args.pass_num:
        logger.info(f"  Running: Pass {args.pass_num} only")
    logger.info("=" * 70)
    ...
    for pass_num in passes_to_run:
        if pass_num not in PASS_CONFIGS:
            logger.error(f"ERROR: Invalid pass number: {pass_num}")
            sys.exit(1)
    ...
    if "merge" in status:
        m = status["merge"]
        logger.info(f"  Merge: {m.get('merged', 0)} ok, {m.get('errors', 0)} errors")
