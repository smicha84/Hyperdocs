...
uses the Opus-based tagger in redo_all_phase1.py instead. This file
...
logger = get_logger("phase1.tag_semantic_primitives")
...
def main():
    ...
    logger.info(f"Loading priority messages ({input_source}) from {TIER4_FILE}...")
    with open(TIER4_FILE, "r") as f:
        tier4_data = json.load(f)

    messages = tier4_data.get("messages", [])
    logger.info(f"Found {len(messages)} priority messages")

    # Also load user messages for supplementary context
    logger.info(f"Loading user messages from {USER_FILE}...")
    with open(USER_FILE, "r") as f:
        user_data = json.load(f)

    user_messages = user_data.get("messages", [])
    logger.info(f"Found {len(user_messages)} user tier2+ messages")
    ...
    logger.info(f"Adding {len(extra_user)} additional user messages from tier2+")
    ...
    if extra_intense:
        logger.info(f"Adding {len(extra_intense)} high-intensity user messages (excl>=5 or caps>0.3) missed by tier system")

    all_messages = messages + extra_user + extra_intense
    all_messages.sort(key=lambda m: m["index"])

    logger.info(f"Total messages to tag: {len(all_messages)}")
    ...
    logger.info(f"Decision Traces: {decision_count} / {len(tagged)}")
    logger.info(f"\nDone.")
