    ...
    for attempt in range(max_retries):
        try:
            ...
            try:
                ...
                logger.error(f"    All JSON recovery strategies failed. Response length: {len(text)}")
                return None
            ...
            if is_transient and attempt < max_retries - 1:
                wait = 30 * (attempt + 1)
                logger.info(f"    API overloaded, retrying in {wait}s (attempt {attempt + 1}/{max_retries})...", end="", flush=True)
                time.sleep(wait)
                continue
            ...
            if is_connection and attempt < max_retries - 1:
                wait = 30 * (attempt + 1)
                logger.info(f"    Connection error, retrying in {wait}s (attempt {attempt + 1}/{max_retries})...", end="", flush=True)
                time.sleep(wait)
                continue
...
def process_session(session_dir, progress):
    ...
    if needs_split:
        ...
        logger.info(f"    [{len(chunks)} chunks, {total_tokens:,} tokens]")
    else:
        chunks = None  # Single prompt

    # ── Pass 1: Thread Analyst ──
    logger.info(f"    Thread Analyst...", end="", flush=True)
    t0 = time.time()
    if chunks:
        chunk_results = []
        for ci, chunk in enumerate(chunks):
            ...
            if r:
                ...
                logger.info(f" c{ci+1}", end="", flush=True)
    ...
    if chunks:
        ...
        for ci, chunk in enumerate(chunks):
            ...
            if r:
                ...
                logger.info(f" c{ci+1}({n})", end="", flush=True)
        ...
        if prim_result:
            ...
            while n_tagged < expected_count and continuation_round < max_continuations:
                ...
                if not remaining_indices:
                    break
                logger.info(f" {n_tagged}/{expected_count}, continuing (round {continuation_round})...", end="", flush=True)
                t1 = time.time()
    ...
    logger.info(f"    Explorer + Verification...", end="", flush=True)
    t0 = time.time()
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("Phase 1 Full Redo — All Sessions, Sequential")
    ...
    if args.session:
        ...
        if skipped_dupes > 0:
            logger.warning(f"Skipped {skipped_dupes} duplicate sessions")
    ...
    logger.info(f"  Successful passes: {progress['totals']['successful_passes']}")
    logger.error(f"  Failed passes: {progress['totals']['failed_passes']}")
    logger.info(f"  Duration: {elapsed/60:.0f} minutes")
