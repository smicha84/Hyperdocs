...
logger = get_logger("phase3.write_more_hyperdocs")
...
for filename, dossier in dossier_map.items():
    if filename in ALREADY_DONE:
        logger.warning(f"  SKIP (already done): {filename}")
        continue

    # Find source file
    source_path = SPECIAL_PATHS.get(filename, V5 / filename)
    if not source_path.exists():
        logger.warning(f"  SKIP (not found): {filename} at {source_path}")
        continue
    ...
    logger.info(f"  {filename}: {len(source_lines)} lines + {block_lines} hyperdoc at line {insertion_point} → {len(result)} lines")
    count += 1
...
for filename in ALREADY_DONE:
    ...
    if src.exists() and not dst.exists():
        ...
        logger.info(f"  COPIED (existing): {filename}")

logger.info(f"\nGenerated {count} new hyperdoc blocks.")
logger.info(f"All files at: {DEST_DIR}")
