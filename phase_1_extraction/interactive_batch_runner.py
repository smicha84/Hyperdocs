...
Usage:
    python3 interactive_batch_runner.py --next       Show the next session to process
    python3 interactive_batch_runner.py --status     Show overall progress
    python3 interactive_batch_runner.py --queue      Show full queue with sizes
"""
import argparse
import json
import os
...
logger = get_logger("phase1.interactive_batch_runner")
...
def show_status():
    ...
    logger.info("=" * 60)
    logger.info("Hyperdocs Batch Orchestrator — Status")
    logger.info("=" * 60)
    ...
    need_p1 = p0 - p1
    need_p2 = p0 - p2
    logger.info(f"Estimated remaining agent launches:")
    logger.info(f"  Phase 1: {need_p1} sessions x 4 agents = {need_p1 * 4}")
    logger.info(f"  Phase 2: {need_p2} sessions x 2 agents = {need_p2 * 2}")
    logger.info(f"  Total: {need_p1 * 4 + need_p2 * 2}")
...
def show_next():
    ...
    logger.info("=" * 60)
    logger.info("Next Sessions to Process")
    logger.info("=" * 60)
    ...
    if need_p1:
        ...
        logger.info(f"  Agent 1: Thread Analyst — read safe files from {base}/")
        logger.info(f"  Agent 2: Geological Reader")
...
def show_queue():
    ...
    for label, group in [("LARGE (500+)", large), ("MEDIUM (100-500)", medium), ("SMALL (<100)", small)]:
        if group:
            ...
            for s in group[:10]:
                logger.info(f"  {s['short']:<16} {s['total']:>6} {s['tier4']:>5} {s['files']:>6}")
            if len(group) > 10:
                logger.info(f"  ... and {len(group) - 10} more")
