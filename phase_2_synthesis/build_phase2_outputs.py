...
logger = get_logger("phase2.build_phase2_outputs")
...
def _extract_key_from_extractions(extractions):
    """Extract key messages from old-format extractions list."""
    key = []
...
def _extract_key_from_canonical(thread_data):
    """Extract key messages from canonical dict-of-categories thread format."""
    nodes = []
    ...
    nodes.sort(key=lambda x: -x['filter_score'])
    return nodes[:8]
...
def extract_key_messages(threads):
    ...
    if 'extractions' in threads:
        return _extract_key_from_extractions(threads['extractions'])
    ...
    if 'threads' in threads:
        ...
        if isinstance(thread_data, dict) and thread_data:
            ...
            if isinstance(first_val, dict) and ('entries' in first_val or 'description' in first_val):
                return _extract_key_from_canonical(thread_data)
...
def process_session(session_dir, force=False):
    ...
    if not any([threads, geo, prims, explorer, summary]):
        logger.info(f"  FAIL {sid}: no Phase 1 files found")
        return 'fail'
    ...
    logger.info(f"  DONE {sid}: {idea_graph['metadata']['total_nodes']} nodes, {len(markers_data['markers'])} markers")
    return 'done'
...
def main():
    ...
    if not sessions:
        logger.info("Usage: python3 build_phase2_outputs.py [--force] session_id1 session_id2 ...")
        sys.exit(1)
    ...
    for sid in sessions:
        ...
        if not os.path.isdir(session_dir):
            logger.info(f"  MISS {sid}: directory not found")
            fail += 1
            continue
