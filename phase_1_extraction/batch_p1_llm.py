...
Usage:
    ...
    python3 batch_p1_llm.py --dry-run

    # Validate on 5 test sessions first
    python3 batch_p1_llm.py --validate

    # Run specific pass across all sessions
    python3 batch_p1_llm.py --pass 1

    # Run all passes across all sessions
    python3 batch_p1_llm.py --all

    # Resume from checkpoint (skips already-complete sessions)
    python3 batch_p1_llm.py --all --resume

    # Show status
    python3 batch_p1_llm.py --status
"""
...
logger = get_logger("phase1.batch_p1_llm")
...
def run_pass_batch(sessions: List[Path], pass_num: int,
    ...
    if not sessions:
        logger.info(f"  Pass {pass_num}: No sessions need processing")
        return {"pass": pass_num, "processed": 0, "success": 0, "failed": 0, "cost": 0.0}

    logger.info(f"\n{'=' * 60}")
    logger.info(f"Pass {pass_num} — {model_name} — {len(sessions)} sessions")
    logger.info(f"Concurrency: {max_workers} | {'DRY RUN' if dry_run else 'LIVE'}")
    logger.info(f"{'=' * 60}")
...
def run_validation(dry_run: bool = False):
    ...
    if not val_sessions:
        logger.error(f"ERROR: No validation sessions found. Checked: {VALIDATION_SESSIONS}")
        logger.info(f"Available sessions: {[s.name for s in sessions[:10]]}...")
        sys.exit(1)

    logger.info(f"\n{'=' * 60}")
    logger.info(f"VALIDATION RUN — {len(val_sessions)} sessions")
    logger.info(f"{'=' * 60}")
    ...
    logger.info(f"\n{'=' * 60}")
    logger.info(f"VALIDATION {'DRY RUN ' if dry_run else ''}COMPLETE — Cost: ${total_cost:.4f}")
    logger.info(f"{'=' * 60}")
...
def run_all_passes(dry_run: bool = False, resume: bool = False):
    ...
    logger.info(f"\n{'=' * 60}")
    logger.info(f"FULL BATCH RUN — {len(sessions)} sessions, 4 passes")
    logger.info(f"{'=' * 60}")
    ...
    if not dry_run:
        ...
        for i, s in enumerate(sessions):
            ...
            if result["success"]:
                ...
                merge_fail += 1
            if (i + 1) % 50 == 0:
                logger.info(f"  Merged {i+1}/{len(sessions)}...")
        logger.error(f"  Merge complete: {merge_success} success, {merge_fail} failed")

    elapsed = time.time() - start_time
    logger.info(f"\n{'=' * 60}")
    logger.info(f"BATCH {'DRY RUN ' if dry_run else ''}COMPLETE")
...
def main():
    ...
    if args.status:
        ...
        if args.pass_num not in [1, 2, 3, 4]:
            logger.error(f"ERROR: Invalid pass number {args.pass_num}. Must be 1-4.")
            sys.exit(1)
        ...
        sessions = get_all_sessions()
        logger.info(f"Merging {len(sessions)} sessions...")
        for i, s in enumerate(sessions):
            result = merge_session(s)
            if (i + 1) % 50 == 0:
                logger.info(f"  {i+1}/{len(sessions)}...")
