...
class MessageMetadata:
    ...
    def to_opus_context(self) -> str:
        ...
        if self.is_emergency_intervention:
            parts.append(f"⚠️EMERGENCY:{self.emergency_reason[:20]}")
...
class SessionMetadata:
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "files_mentioned": self.all_files_mentioned[:50],  # Top 50
            "file_mention_counts": dict(sorted(self.file_mention_counts.items(), key=lambda x: -x[1])[:20]),
            "most_mentioned_files": self.most_mentioned_files[:10],
            ...
            "most_accessed_files": dict(sorted(self.file_access_counts.items(), key=lambda x: -x[1])[:10]),
            # Frustration
            ...
            "emergency_reasons": self.emergency_reasons[:20],  # Top 20 reasons
...
class MetadataExtractor:
    ...
    def extract_message_metadata(self, msg: GeologicalMessage, index: int) -> MessageMetadata:
        ...
        if content:
            ...
            if meta.has_repeated_phrase:
                ...
                meta.emergency_reason = f"REPEAT({meta.repeat_count}x):{meta.repeated_phrase[:20]}"
    ...
    def extract_session_metadata(self, session: GeologicalSession, file_name: str) -> SessionMetadata:
        ...
        meta.most_mentioned_files = sorted(file_counts, key=file_counts.get, reverse=True)[:10]
