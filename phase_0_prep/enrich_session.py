...
# Works both as `python3 phase_0_prep/enrich_session.py` and when imported as module
try:
...
def main():
    logger.info("=" * 60)
    logger.info("Phase 0: Deterministic Prep (Pure Python, $0)")
    logger.info("=" * 60)
    ...
    if SESSION_FILE is None or not SESSION_FILE.exists():
        logger.error(f"ERROR: Session file not found: {SESSION_FILE}")
        logger.info(f"  Set HYPERDOCS_CHAT_HISTORY or check HYPERDOCS_SESSION_ID={SESSION_ID}")
        sys.exit(1)
    ...
    try:
        ...
        logger.error(f"ERROR: Exception while parsing session file: {type(e).__name__}: {e}")
        sys.exit(1)
    if not session:
        ...
        try:
            ...
            logger.error(f"ERROR: Could not parse session file: {SESSION_FILE}")
            logger.info(f"  File size: {size:,} bytes, total lines: {lines}, non-empty lines: {json_lines}")
            if size == 0:
                logger.info(f"  Cause: file is empty")
            elif json_lines == 0:
                logger.info(f"  Cause: no non-empty lines")
            else:
                logger.info(f"  Cause: no lines parsed into valid messages (check JSONL format)")
        except Exception:
            logger.error(f"ERROR: Could not parse session file: {SESSION_FILE}")
        sys.exit(1)
    ...
    for idx, msg in enumerate(session.messages):
        ...
        # Progress indicator
        if (idx + 1) % 100 == 0:
            logger.info(f"  Processed {idx + 1}/{len(session.messages)} messages...")
    ...
    output = {
        ...
        "generator": "enrich_session.py (Phase 0)",
        "session_stats": stats,
    ...
    logger.info(f"Messages:       {len(enriched_messages)}")
    logger.info(f"  User:         {stats['user_messages']}")
    logger.info(f"  Assistant:    {stats['assistant_messages']}")
    logger.info(f"Tier distribution:")
    for tier, count in stats["tier_distribution"].items():
        pct = count / len(enriched_messages) * 100 if enriched_messages else 0
        logger.info(f"  {tier}: {count} ({pct:.0f}%)")
    logger.info(f"Frustration peaks:     {len(stats['frustration_peaks'])}")
    logger.info(f"Emergency interventions: {len(stats['emergency_interventions'])}")
    logger.info(f"Unique files mentioned:  {len(stats['file_mention_counts'])}")
    logger.info(f"Errors detected:         {stats['error_count']}")
    # Phase 0 data quality metrics
    logger.info(f"\nData quality fixes applied:")
    logger.info(f"  Protocol messages:     {stats['protocol_messages']} (system-generated, not human)")
    logger.info(f"  Human messages:        {stats['human_messages']} (actual human input)")
    logger.info(f"  Char-per-line fixed:   {stats['char_per_line_messages']} messages")
    logger.info(f"  Tool failures:         {stats['tool_failure_count']}")
    logger.info(f"  Subagent session:      {stats['is_subagent']} (agent_id: {stats['agent_id']})")

    if stats["top_files"]:
        logger.info(f"\nTop 10 most-mentioned files:")
        for fname, count in stats["top_files"][:10]:
            logger.info(f"  {count:3d}x  {fname}")

    logger.info(f"\nToken usage (session total):")
    logger.info(f"  Input:    {stats['total_input_tokens']:,}")
    logger.info(f"  Output:   {stats['total_output_tokens']:,}")
    logger.info(f"  Thinking: {stats['total_thinking_chars']:,} chars")
