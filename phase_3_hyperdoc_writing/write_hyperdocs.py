...
def compose_hyperdoc_block(
    ...
    lines.append(f"# Session: conv_3b7084d5 | Generated: {datetime.utcnow().strftime('%Y-%m-%d')}")
    lines.append(f"# ===========================================================================")
    lines.append(f"#")
    ...
    for item in friction_items:
        ...
        lines.append(f"# @ctx:trace=conv_3b7084d5:msg{item['msg_ref']}")
    ...
    for w in warnings:
        ...
        lines.append(f"# @ctx:trace=conv_3b7084d5:msg{w.get('first_discovered', '?')}")
