...
logger = get_logger("tools.analyze_data_flow")
...
def get_phase_for_path(filepath: Path) -> tuple:
    ...
    Returns (phase_number, phase_name) where phase_number is:
      ...
      4 for phase_4a_aggregation/ or phase_4_insertion/
    ...
    phase_map = {
        ...
        "phase_4a_aggregation": (4, "Phase 4"),
...
def discover_python_files() -> list:
    ...
    for root, dirs, files in os.walk(REPO):
        ...
        for fname in sorted(files):
            if fname.endswith(".py") and fname != "analyze_data_flow.py":
...
def analyze():
    ...
    result = {
        ...
        "generator": "analyze_data_flow.py",
    ...
    for out_file, script in sorted(producers.items()):
        ...
        logger.info(f"    writes: {out_file}{status}")

    logger.info(f"\n{'=' * 70}")
    logger.info(f"DEAD ENDS ({len(dead_ends)} output files with no consumers)")
    logger.info("=" * 70)
    if dead_ends:
        for de in dead_ends:
            producer = producers.get(de, "unknown")
            logger.info(f"  {de}")
            logger.info(f"    produced by: {producer}")
    else:
        logger.info("  None found.")

    logger.info(f"\n{'=' * 70}")
    logger.info(f"PHASE POSITIONING ({len(mispositioned)} potentially mispositioned scripts)")
    logger.info("=" * 70)
    if mispositioned:
    ...
    logger.info(f"\n{'=' * 70}")
    logger.info("PER-SCRIPT I/O MAP")
    logger.info("=" * 70)
        ...
        for script_name, info in scripts_in_phase:
            ...
            if info["inputs"]:
                for inp in info["inputs"]:
                    ...
                    logger.info(f"    <- {inp} (from Phase {src_phase})")
            else:
                logger.info(f"    <- (no pipeline JSON inputs detected)")
            if info["outputs"]:
                for out in info["outputs"]:
                    is_dead = " [DEAD END]" if out in dead_ends else ""
                    logger.info(f"    -> {out}{is_dead}")
            else:
                logger.info(f"    -> (no pipeline JSON outputs detected)")

    logger.info(f"\n{'=' * 70}")
    logger.info("ANALYSIS STATS")
    logger.info("=" * 70)
    logger.info(f"  Total scripts analyzed: {len(scripts)}")
    logger.info(f"  Total output files tracked: {len(producers)}")
    logger.info(f"  Dead ends: {len(dead_ends)}")
    logger.info(f"  Mispositioned scripts: {len(mispositioned)}")
